...
a.stopPropagation();b.init().open();return!1});b.on("insert",function(a){var b="![%alt](%url)";a.url?(b=b.replace("%alt",a.alt),b=b.replace("%url",a.url),c.insert(b)):c.f(a.id)});c.a=b},f:function(c){var a=this,b;b=refinery("admin.ImageDialog",{url:"/refinery/dialogs/image/"+c,buttons:[{text:"Insert",click:function(){b.insert()}},{text:"Back to the library",click:function(){b.close();a.a.open()}}]}).init();b.on("insert",function(d){var c;c="![%alt](%url)".replace("%alt",d.alt);c=c.replace("%url",
d.sizes[d.size]);a.insert(c);b.destroy(!0)});b.open()},i:function(){var c=this,a=c.b,b,a=$("<button/>",{title:t("refinery.editor.resources_dialog_button_title"),"class":"editor-resources-dialog-btn refinery-btn"}).prependTo(a);b=refinery("admin.ResourcesDialog");a.on("click",function(a){a.preventDefault();a.stopPropagation();b.init().open();return!1});b.on("insert",function(a){c.insert('<a href="'+a.url+'" class="file '+a.ext+'" title="'+a.name+" - "+a.size+' bytes " >'+a.name+"</a>")});c.c=b},h:function(){var c=
...
a.editor.remove(a.options.file.name),a.editor.load(),a.b=$(a.editor.getElement("wrapper")).find("#epiceditor-utilbar"),a.e=$("<button/>",{"class":"epiceditor-toggle-button",text:t("refinery.editor.toggle_editor")}).appendTo(c),a.e.on("click",function(c){var e=b.val();c.preventDefault();d.toggle();b.toggle();b.is(":visible")?b.val(e):a.editor.importFile(a.options.file.name,e)}),a.g(),a.i(),a.h(),a.is({initialised:!0,initialising:!1}),a.trigger("init"));return a}});
refinery.admin.ui.editorEpicEditor=function(c){var a=c.find(".ui-tabs"),b=[];c.find("textarea.replace-with-editor").each(function(){var a=$(this).parent();b[b.length]=refinery("editor.EpicEditor").init(a)});a.on("tabsactivate",function(){for(var a=b.length-1;0<=a;a--)b[a].editor&&b[a].editor.reflow()})};}(window, jQuery));
